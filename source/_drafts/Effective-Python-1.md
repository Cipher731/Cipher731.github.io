...
第二个问题是在操作文件的时候，Python 3的文件打开操作分为有文本模式`t`和二进制模式`b`，前者以`str`承载`read`和`write`等文件读写操作，后者则使用`bytes`。

str模式，以`r`,`w`或`a`等模式（不加`b`，或显式指定`t`）打开的话，相当于是自动嵌套了一层`encode`和`decode`，从文件读取内容时会对文件中原始存储的字节数据进行一次`decode`，编码由调用`open`时传入的`encoding`命名参数指定，不指定的情况下回使用OS的默认编码方案，因此在Linux上处理某些来自中文版本Windows的文件时，就会以UTF-8编码打开然后报错或乱码。

bytes模式，以`rb`,`wb`或`ab`等模式（加了`b`）打开的话，就会直接对文件的原始内容进行操作。

### 1.4 用支持插值的`f-string`替代C风格的格式字符串与`str.format`方法
这一条详细介绍了C风格的`%`格式化，Python 3的`str.format`方法以及推荐使用的`f-string`。虽然篇幅很多，但是没有涉及到我最关心的性能问题，都只是从编码的易用性和可维护性进行了阐述，所以就挖个坑分析一下几种方法的性能和对应的底层原理。

`f-string`最大的优点是可读性，就和很多年前开始就已经被广泛应用的HTML模板类似，能清楚地看到什么值被插到什么地方，不会出现`%`方法和`str.format`方法要同时移动`{}`位置和变量等情况。
